...
def perform_relative_risk():
    try:
        ...
        # 1. Parse Input into 2x2 DataFrame
        ...
        if confidence_level is not None:
            try:
                ...
                z = norm.ppf(1 - (1 - ci) / 2)
                ci_lower = np.exp(np.log(relative_risk) - z * se_log_rr)
                ci_upper = np.exp(np.log(relative_risk) + z * se_log_rr)
        ...
        if alpha is not None:
            try:
                ...
                power = NormalIndPower().power(effect_size=effect_size, nobs1=a + b, alpha=alpha)
                power_result = {round(alpha, 3): round(power, 3)}
            except Exception as e:
                logger.warning(f"Power calculation failed: {str(e)}")

        conclusion = (
            "The likelihood of the outcome is greater in the treatment group."
        ...
        logger.info(f"Relative Risk result: {result}")
