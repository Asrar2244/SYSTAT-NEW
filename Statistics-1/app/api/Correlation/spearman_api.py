...
from app.api.helpers.constant import (
    ...
    PEARSON_TEST_LOG_FILE_PATH  # You can change to SPEARMAN_TEST_LOG_FILE_PATH if preferred
)
from app.api.helpers.logger import Logger

spearman_api = Blueprint('spearman_api', __name__)

# Same Mardia and Henze-Zirkler functions from your Pearson API
...
@spearman_api.route("/spearman-correlation", methods=["POST"])
def spearman_correlation():
    ...
    try:
        ...
        if result_display_format == "matrix":
            ...
            interpretation = (
                f"Spearman Rank Correlation\t{date_now}\n\n"
                f"{source_info}\n\n"
            )
        ...
        return jsonify({
            "success": True,
            "results": results,
            "assumption_checking": assumption_results or None
        })

    except ValueError as e:
        logger.log(LOG_VALUE_ERROR.format(str(e)))
        return jsonify({"success": False, "error_message": VALUE_ERROR_MSG.format(str(e)), "error_code": "VALUE_ERROR"}), 400
    except KeyError as e:
        logger.log(LOG_KEY_ERROR.format(str(e)))
        return jsonify({"success": False, "error_message": KEY_ERROR_MSG.format(str(e)), "error_code": "KEY_ERROR"}), 400
    except TypeError as e:
        logger.log(LOG_TYPE_ERROR.format(str(e)))
        return jsonify({"success": False, "error_message": TYPE_ERROR_MSG.format(str(e)), "error_code": "TYPE_ERROR"}), 400
    except Exception as e:
        logger.log(LOG_UNEXPECTED_ERROR.format(str(e)))
        return jsonify({"success": False, "error_message": UNEXPECTED_ERROR_MSG.format(str(e)), "error_code": "UNEXPECTED_ERROR"}), 500
