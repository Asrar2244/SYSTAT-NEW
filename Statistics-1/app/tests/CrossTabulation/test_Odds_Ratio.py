...
class TestOddsRatioAPI:
    ...
    def test_valid_odds_ratio_wide_format(self, client):
        """Test Odds Ratio with valid 2x2 contingency table (Wide Format)."""
        response = client.post('/odds-ratio', json={
            "DB": True,
            "data": [[30, 20], [50, 40]],
        ...
        assert "Odds Ratio" in data
        assert "P-Value" in data
        assert "Chi-square Statistic" in data
    
    def test_valid_odds_ratio_long_format(self, client):
        """Test Odds Ratio with valid categorical data (Long Format)."""
        response = client.post('/odds-ratio', json={
            "DB": False,
            "data": [
